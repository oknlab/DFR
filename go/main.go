...
func redisSetEx(k,v string) { _,_ = redis("SETEX",k,"900",v) }
...
func pipelineHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.UseFirecrawl && req.Query!="" { search["firecrawl"] = searchFirecrawl(req.Query, req.MaxURLs) }
	...
	resp:=map[string]any{"product":"distributed-web-data-os","stages":map[string]any{"search":search,"crawl":results,"scrape":results},"urls":final,"apify_enabled":apifyToken!=""}
...
func health(w http.ResponseWriter, r *http.Request){ _ = json.NewEncoder(w).Encode(map[string]any{"status":"ok","redis":redisPing()}) }
func openapi(w http.ResponseWriter, r *http.Request){ _,_=w.Write([]byte(`{"openapi":"3.0.3","info":{"title":"Distributed Web Data OS","version":"1.0.0"},"paths":{"/pipeline":{"post":{"responses":{"200":{"description":"ok"}}}},"/health":{"get":{"responses":{"200":{"description":"ok"}}}},"/ui":{"get":{"responses":{"200":{"description":"ok"}}}}}}`)) }
func ui(w http.ResponseWriter, r *http.Request){ _,_=w.Write([]byte(`<html><body><h3>Distributed Web Data OS UI</h3><input id=q><button onclick='go()'>Run</button><pre id=o></pre><script>async function go(){let q=document.getElementById('q').value;let r=await fetch('/pipeline',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({query:q,seed_urls:['https://example.com'],max_urls:3,use_firecrawl:false})});document.getElementById('o').textContent=JSON.stringify(await r.json(),null,2)}</script></body></html>`)) }

func main(){ http.HandleFunc("/pipeline", pipelineHandler); http.HandleFunc("/health", health); http.HandleFunc("/openapi.json", openapi); http.HandleFunc("/ui", ui); log.Fatal(http.ListenAndServe(":10000", nil)) }
